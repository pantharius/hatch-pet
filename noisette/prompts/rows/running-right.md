Create a single horizontal sprite strip for the Codex app digital pet `noisette` in the state `running-right`.
...
Output exactly 8 separate animation frames arranged left-to-right in one single row. Each frame must show the same pet: a friendly chibi squirrel, warm brown fur, cream belly, oversized curled tail, tiny rounded ears, bright black eyes, small acorn held close to the body.

Style contract: Codex digital pet sprite style: pixel-art-adjacent low-resolution mascot sprite, compact chibi proportions, chunky whole-body silhouette, thick dark 1-2 px outline, visible stepped/pixel edges, limited palette, flat cel shading with at most one small highlight and one shadow step, simple readable face, tiny limbs, and no detail that disappears at 192x208. Avoid polished illustration, painterly rendering, anime key art, 3D render, vector app-icon polish, glossy lighting, soft gradients, realistic fur or material texture, anti-aliased high-detail edges, and complex tiny accessories. Additional user style notes: Codex digital pet sprite style, pixel-art-adjacent, thick dark outline, limited warm brown and cream palette, flat cel shading, no realistic fur, no scenery, no shadows.
...
Critical correction from prior failed output:
- The previous row visually duplicated/overlapped the squirrel across neighboring cells. Do not repeat that mistake.
- Each invisible 192x208 slot must contain exactly one squirrel and no part of another squirrel.
- Keep the entire curled tail, body, paws, and acorn inside that slot with clear transparent margin on the left and right.
- The sprite should read as a compact right-facing/run-to-the-right pose cycle, not as several squirrels blended together.
- Prefer smaller, tighter poses over large stretched poses if needed to keep every frame self-contained.
...
- Maintain at least 12 px of empty magenta padding between the pet silhouette and each slot's left and right boundaries.
- Do not let tails, ears, paws, the acorn, outlines, or any detached pixels enter an adjacent slot.
